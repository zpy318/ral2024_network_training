...
class Dataset(Dataset):
    """Camera_tool_tip_and_base_prediction dataset Preparation."""
    ...
    def __getitem__(self, idx):
        ...
        label[0][0] = label[0][0] * 640 / 800
        label[0][1] = label[0][1] * 480 / 600
        label[1][0] = label[1][0] * 640 / 800
        label[1][1] = label[1][1] * 480 / 600
...
class RandomSizedBBoxSafeCrop(object):
    ...
    def __init__(self, p = 0.8):
        ...
        self.ssr = abm.RandomSizedBBoxSafeCrop(width=640, height=480, erosion_rate=0.1, interpolation=1, 
                                               always_apply = True)
        self.centerCrop = abm.CenterCrop (height = 480, width = 640, always_apply=True)
...
class Resnet_tool_tip_labelling_large(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    # decoder 1
    x113 = self.dec11(x13) # 64 x 120 x 160
    ...
    x114 = self.dec12(x113) # 32 x 240 x 320
    ...
    x115 = self.dec13(x114) # 16 x 480 x 640
    ...
    # decoder 2
    x213 = self.dec21(x13) # 64 x 120 x 160
    ...
    x214 = self.dec22(x213) # 32 x 240 x 320
        ...
        for phase in ['train', 'val']:
            ...
            for batch_idx, data in enumerate(progress_bar):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    current_argmax1 = np.array(np.unravel_index(outputs_flatten1.cpu().argmax(axis=1), (480,640)))
                    current_argmax2 = np.array(np.unravel_index(outputs_flatten2.cpu().argmax(axis=1), (480,640)))
                    l3label = np.ones((outputs1.size(0),)) * 50
                    ...
                    for i in range(len(outputs_np1)):
                        ...
                        current_argmax = np.unravel_index(a.argmax(), (480,640))
...
if __name__=="__main__": 
    ...
    train_dataset = Dataset(img_dir = "../../dataset/camera_tool_tip_and_base_prediction_input_600_800_crop_to_480_640/train_img", csv_dir = "../../dataset/camera_tool_tip_and_base_prediction_input_600_800_crop_to_480_640/train_tool_tip_and_base_points.csv",  
                            transform = data_transform_train) #= data_transform)

    validation_dataset = Dataset(img_dir = "../../dataset/camera_tool_tip_and_base_prediction_input_600_800_crop_to_480_640/validation_img", csv_dir = "../../dataset/camera_tool_tip_and_base_prediction_input_600_800_crop_to_480_640/validation_tool_tip_and_base_points.csv",  
